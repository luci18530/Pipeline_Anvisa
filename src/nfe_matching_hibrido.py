...
# Caminhos
INPUT_ZIP = BASE_DIR / 'data' / 'processed' / 'df_etapa14_final_enriquecido.zip'
OUTPUT_DIR = BASE_DIR / 'data' / 'processed'
OUTPUT_ZIP = OUTPUT_DIR / 'df_etapa15_resultado_matching_hibrido.zip'

# Base mestre ANVISA
BASE_MESTRE_PATH = BASE_DIR / 'output' / 'anvisa' / 'baseANVISA.csv'
...
def carregar_base_mestre():
    ...
    if not BASE_MESTRE_PATH.exists():
        raise FileNotFoundError(f"Base mestre nao encontrada: {BASE_MESTRE_PATH}")
    
    df = pd.read_csv(BASE_MESTRE_PATH, sep='\t', dtype=str, low_memory=False)
    
    print(f"[OK] Base mestre carregada: {len(df):,} registros")
