...
def _resolver_base_anvisa_path():
    """Retorna o caminho disponível para baseANVISA.csv com fallback legível."""
    candidatos = [
        OUTPUT_DIR / 'anvisa' / 'baseANVISA.csv',
        OUTPUT_DIR / 'baseANVISA.csv'
    ]

    for caminho in candidatos:
        if caminho.exists():
            if caminho == candidatos[1]:
                print(
                    "[AVISO] Base ANVISA encontrada em output/baseANVISA.csv. "
                    "Considere movê-la para output/anvisa/baseANVISA.csv para aderir à reorganização."
                )
            return caminho

    raise FileNotFoundError(
        "Base ANVISA não encontrada. Coloque o arquivo em output/anvisa/baseANVISA.csv "
        "(ou, provisoriamente, output/baseANVISA.csv)."
    )
