...
- Vistoria posterior da listagem de sobrepreco apontou novo falso positivo: `100 MG / 5 ML` na descricao NFe contra `100 MG/ML` na apresentacao CMED. A Etapa 17.5 agora extrai concentracao explicita `MG/ML` de ambos os textos e bloqueia divergencias acima de 25% com `observacao_conversao_unidade=dosagem_nfe_cmed_incompativel`. A auditoria completa foi exportada para `data/processed/auditoria_etapa17_5_unidade_caixa.xlsx`.
- Refinamento adicional: a decisao caixa/unidade agora considera contexto operacional de produto, nao apenas `unidade` e fator. Foram adicionadas as colunas `contexto_produto_unidade`, `confianca_contexto_produto` e `sinais_contexto_produto`, usando descricao, quantidade, preco unitario, apresentacao, `TIPO DE PRODUTO`, `CLASSE TERAPEUTICA`, `GRUPO TERAPEUTICO` e `GRUPO ANATOMICO`. Unidades genericas como `UN/UND` so convertem com contexto suficiente; casos ambiguos ficam marcados como `contexto_insuficiente_unidade_generica` ou `descricao_sugere_apresentacao_inteira`.
- Foi criado o script reproduzivel `pipelines/nfe/scripts/gerar_auditoria_etapa17_5_unidade_caixa.py`, que gera o Excel `data/processed/auditoria_etapa17_5_unidade_caixa.xlsx` com abas de contextos, conversoes aplicadas, bloqueadas, top sobrepreco e listagem completa.
